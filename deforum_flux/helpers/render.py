    ...
    if anim_args.interpolate_key_frames:
        for i in range(len(prompts_c_s)-1):
            ...
            for j in range(dist_frames):
                ...
                t = j * 1.0 / dist_frames
                args.init_c = {k:interpolate(t, prompt1_c[k], prompt2_c[k], mode="slerp") for k in prompt1_c.keys()}
        ...
        for i in range(len(prompts_c_s)-1):
            for j in range(anim_args.interpolate_x_frames+1):
                ...
                prompt1_c = prompts_c_s[i]
                prompt2_c = prompts_c_s[i+1]  
                args.init_c = {k:prompt1_c[k].add(prompt2_c[k].sub(prompt1_c[k]).mul(j * 1/(anim_args.interpolate_x_frames+1))) for k in prompt1_c.keys()}
